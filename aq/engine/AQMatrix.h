...
protected:
  ...
  uint64_t totalCount;
  uint64_t nbRows;
  std::string answerFormatC;
  std::string answerFormatP;
  size_t nbRowsParsed;
  size_t nbPacket;
  size_t packet;
...
{
  boost::scoped_array<char> answerData(new char[this->answerFormatC.size() + 128]); // FIXME
  sprintf(answerData.get(), this->answerFormatC.c_str(), this->packet);
